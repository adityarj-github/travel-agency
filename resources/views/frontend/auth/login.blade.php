...
@section('title', 'Sign In — ' . setting('site_name', config('app.name')))
    ...
    <section class="relative overflow-hidden bg-slate-50 py-12 sm:py-16 lg:py-20">
        ...
        <div class="container relative">
            <div class="mx-auto grid max-w-5xl overflow-hidden rounded-3xl border border-slate-100 bg-white card-shadow lg:grid-cols-2">
                ...
                <div class="relative hidden min-h-[560px] flex-col justify-between overflow-hidden bg-slate-900 p-10 text-white lg:flex">
                    <img src="https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?auto=format&fit=crop&w=1200&q=80"
                         alt="Mountain landscape" class="absolute inset-0 h-full w-full object-cover opacity-60">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="mt-3 max-w-sm text-sm text-white/80">
                            Sign in to manage your bookings, revisit saved tours, and pick up planning right where you left off.
                        </p>

                        <ul class="mt-8 space-y-4 text-sm">
                            @foreach ([
                                ['M5 13l4 4L19 7', 'Track and manage all your bookings'],
                                ['M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z', 'Save your favourite destinations'],
                                ['M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', 'Download trip vouchers instantly'],
                            ] as [$icon, $label])
                ...
                <div class="flex flex-col justify-center p-8 sm:p-12">
                    <div class="mx-auto w-full max-w-sm">
                        ...
                        <h1 class="text-2xl font-bold text-slate-900 sm:text-3xl">Welcome back</h1>
                        <p class="mt-2 text-sm text-slate-500">Sign in to continue to your account.</p>

                        <form method="POST" action="{{ route('login.attempt') }}" class="mt-8 space-y-5" x-data="{ show: false }">
                            @csrf
                        ...
                        <div class="my-7 flex items-center gap-3 text-xs text-slate-400">
                            ...
                            NEW TO {{ strtoupper(setting('site_name', config('app.name'))) }}?
                            <span class="h-px flex-1 bg-slate-200"></span>
                        </div>

                        <a href="{{ route('register') }}" class="btn-outline w-full">Create an account</a>
                    </div>
                </div>
